...
def de_xor_data(data):
    data = bytearray(data)
    key = data[0x10]
    search_pattern = bytes([0x76, 0x36, 0x76, 0x36, 0x35, 0x35, 0x35, 0x76, 0x36, 0x76, 0x36, 0x3D, 0x3D, 0x3D, 0xD7, 0xE8, 0xD6, 0xB5, 0x0A])
...
def extract_encrypted_blocks(data):
    ...
    main_data_blocks_size = struct.unpack('<I', data[current_pointer:current_pointer + 4])[0]
    current_pointer += 4
    ...
    while current_pointer < 0x44 + main_data_blocks_size:
        ...
        current_pointer += 4
        if block_type == b'\x07':
            encrypted_blocks.append(data[current_pointer:current_pointer + block_size])
...
def decrypt_file(data):
    ...
    while current_pointer < 0x44 + main_data_blocks_size:
        ...
        current_pointer += 4
        if block_type == b'\x07':
            decrypted_data = decrypt_with_des(data[current_pointer:current_pointer + block_size])
            data = data[:current_pointer] + decrypted_data + data[current_pointer + block_size:]
            current_pointer += block_size
        else:
            current_pointer += block_size
...
def main(input_file, mode):
    ...
    if mode == "-e":
        ...
        output_directory = os.path.join(os.path.dirname(input_file), f"{output_file}_decrypted_blocks")
        ...
        for index, block in enumerate(encrypted_blocks):
            ...
            with open(os.path.join(output_directory, f"{output_file}_block_{index}.decrypted.dat"), 'wb') as f:
...
def parse_arguments():
    ...
    parser.add_argument("-e", action="store_true", help="Extract and decrypt blocks from the input file.")
    parser.add_argument("-d", action="store_true", help="Decrypt the entire input file.")
    parser.add_argument("-f", "--file", required=True, help="Path to the input PCB file.")
...
if __name__ == "__main__":
    ...
    if args.e or args.d:
        ...
        print("Invalid mode. Use -e to decrypt and extract blocks, or -d to decrypt and make decrypted .pcb.")
